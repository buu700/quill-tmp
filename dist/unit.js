    ...
    toolbar: {
      handlers: {
        ...
        image: function image() {
          ...
          if (fileInput == null) {
            var accept = 'image/png, image/gif, image/jpeg, image/bmp, image/x-icon';
            fileInput = document.createElement('input');
            fileInput.setAttribute('type', 'file');
            fileInput.setAttribute('accept', accept);
            fileInput.classList.add('ql-image');
            var uploadImage = function uploadImage(dataURI) {
              var range = _this3.quill.getSelection(true);
              _this3.quill.updateContents(new _quillDelta2.default().retain(range.index).delete(range.length).insert({ image: dataURI }), _emitter2.default.sources.USER);
              _this3.quill.setSelection(range.index + 1, _emitter2.default.sources.SILENT);
            };
            if (_typeof(window.cordova) === 'object' && _typeof(window.chooser) === 'object' && typeof window.chooser.getFile === 'function') {
              fileInput.addEventListener('click', function (e) {
                e.preventDefault();
                window.chooser.getFile(accept).then(function (file) {
                  if (file) {
